...
export default function Register({ navigation }) {
    ...
    const response = await api
      ...
      .catch((error) => {
        if (error.response.status === 400) {
          Alert.alert(
            `Olá, ${name}`,
            "Esse e-mail já está em uso, tente novamente utilizando outro!"
          );
        }
  ...
  const [logo] = useState(
    new Animated.ValueXY({ x: height * 0.176, y: height * 0.176 })
  );
  ...
  return (
    <Container source={require("../../assets/background.png")}>
      <View style={{ bottom: height * 0.15, alignItems: "center" }}>
        ...
        <View
          style={{ top: height * 0.08, alignItems: "center" }}
        >
          <TextInput
            value={email}
            allowFontScaling={false}
          ...
          <View style={{ top: height * 0.066 }}>
            ...
            <Button
              ...
              style={{ alignItems: "center", justifyContent: "center", top: height * 0.035 }}
            >
              <Text  allowFontScaling={false} style={{ fontFamily: "Nunito_700Bold" }} color={colors.backgroundLogin} fontSize={height * 0.018}>
                CADASTRAR-SE
              </Text>
            </Button>
            <TouchableOpacity style={{top: height * 0.05}} onPress={() =>  navigation.navigate('Login')}>
              <Text
                allowFontScaling={false}
                texttransform="none"
